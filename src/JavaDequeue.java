...
public class JavaDequeue {
	...
	public static void main(String... args) {
		try (var in = new Scanner(System.in)) {
			...
			int u = 0;
			int idx = 0;
			var set = new TreeSet<>(); // Optimisation 1: TreeSet is more efficient than a HashSet because it is
										// ordered,
			...
			for (int i = 0; i < n; i++, idx++) {
				...
				if (idx == m - 1) {
					...
					if (sz > u) {
						u = sz;
						if (u == m) {
							out.print(u); // Optimisation 2: Max no of different integers we can have in an array of
											// size m
							// so if we got the max value before we traversed all n elements, we return,
							// because clearly there no need run the program further
